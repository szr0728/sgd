/**
 https://gitee.com/lrd110/tree_table_treegrid_based_on_layui
...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function (exports) {
    ...
    Class.prototype.filterArray = function (data, field_Id, field_upId) {
        ...
        for (var i = 0; i < data.length; i++) {
            var n = data[i];
            if (data[i].isOpen == undefined || data[i].isOpen == null) {
                n.isOpen = true;
            } else {
                n.isOpen = data[i].isOpen;
            }
        }
        ...
        var that = this
            ...
            , render = function () { //后续性能提升的重点
                ...
                layui.each(data, function (i1, item1) {
                    ...
                    var tempOpen = true;
                    var pid = 0;
                    if (item1[options.treeUpId] != null) {
                        for (var i = 0; i < data.length; i++) {
                            if (item1[options.treeUpId] == data[i][options.treeId]) {
                                tempOpen = data[i].isOpen;
                                break;
                            }
                        }
                        pid = item1[options.treeUpId];
                    } else {
                        pid = item1[options.treeId];
                    }

                    if (tempOpen) {
                        trs.push('<tr data-index="' + i1 + '" temppid="pid_' + pid + '" upids="' + item1["upIds"] + '">' + tds.join('') + '</tr>');
                    } else {
                        trs.push('<tr style="display:none" data-index="' + i1 + '" temppid="pid_' + pid + '" upids="' + item1["upIds"] + '">' + tds.join('') + '</tr>');
                    }
                    //                    trs.push('<tr style="display: ' + item1["isOpen"] ? "" : "none" + ';" data-index="' + i1 + '" upids="' + item1["upIds"] + '">' + tds.join('') + '</tr>');

                    if (tempOpen) {
                        trs_fixed.push('<tr data-index="' + i1 + '">' + tds_fixed.join('') + '</tr>');
                    } else {
                        trs_fixed.push('<tr style="display:none" data-index="' + i1 + '">' + tds_fixed.join('') + '</tr>');
                    }
                    //trs_fixed.push('<tr data-index="' + i1 + '">' + tds_fixed.join('') + '</tr>');
                    ...
                    if (tempOpen) {
                        ...
                        trs_fixed_r.push('<tr style="display:none" data-index="' + i1 + '">' + tds_fixed_r.join('') + '</tr>');
                    }
                    //trs_fixed_r.push('<tr data-index="' + i1 + '">' + tds_fixed_r.join('') + '</tr>');
                });
    ...
    Class.prototype.events = function () {
        ...
        that.elem.on('click', 'i.layui-tree-head', function () {
            ...
            var sonO1 = $("[temppid=pid_" + o[options.treeId] + "]");
            ...
            //    o.children[i].isOpen = false;

            //    for (var j = 0; j < o.children[i].children.length; j++) {
            //        o.children[i].children[j].isOpen = false;

            //        for (var k = 0; k < o.children[i].children[j].children.length; k++) {
            //            o.children[i].children[j].children[k].isOpen = false;
            //        }
            //    }
            //}
            //sonO.find("i");
            ...
            if (o.isOpen) {//打开状态
                ...
                sonO.find("td .layui-tree-head").each(function () {
                    if ($(this).html() !== "&nbsp;&nbsp;") {
                        $(this).html('&#xe623;');
                    }
                });
